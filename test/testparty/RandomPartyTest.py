...
class RandomPartyTest(unittest.TestCase):
    ...
    mopacSettings = Settings(PARTY1, PROFILE, mopacProtocol, progress,
                             parameters)
    serialized = Path("test/resources/testprofile.json").read_text("utf-8")
    profile: UtilitySpace = pyson.parse(json.loads(serialized),
                                        LinearAdditive)  #type:ignore
    ...
    def testSendInfo(self):
        settings = Settings(self.PARTY1, self.profileref, self.protocolref,
                            self.progress, self.parameters)
